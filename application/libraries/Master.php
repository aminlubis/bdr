...
final Class Master {
	...
	public function get_format_no_kta($data){

		$CI =&get_instance();
		$db = $CI->load->database('default', TRUE);
		$qry = $db->where('jenis_anggota', $data['jenis_anggota'])->where('tgl_verifikasi IS NOT NULL')->get('t_anggota')->num_rows();
		/*plus 1*/
		$max_num = $qry + 1 ;
		// print_r($max_num);die;
		/* {jenis_anggota} - {domisili} - {tgl_verifikasi} - {no_urut_anggota} */
		if($data['jenis_anggota'] == 'BU'){
			$format = $data['kepengurusan'].'-'.$data['penempatan_kota'].'-'.$CI->tanggal->dateToString($data['tgl_verifikasi']).'-00'.$max_num;
		}

		if($data['jenis_anggota'] == 'LKRI'){
			$format = 'LKRI-'.$data['kepengurusan'].'-'.$data['penempatan_kota'].'-'.$CI->tanggal->dateToString($data['tgl_verifikasi']).'-00'.$max_num;
		}
