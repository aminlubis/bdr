...
final Class Graph_master {
    ...
    function setting_module($params) {
		...
		if($params['prefix']==1){	
			...
			$fields = array('Anggota'=>'total');
			$title = '<span style="font-size:13.5px">Grafik Data Bencana</span>';
			$subtitle = 'Source: Sistem Keanggotaan Bhayangkara Utama';
		...
		if($params['prefix']==4){	
			...
			$title = '<span style="font-size:13.5px">Grafik Data Anggota Berdasarkan Sebaran Provinsi</span>';
			$subtitle = 'Source: Sistem Keanggotaan Bhayangkara Utama ';
			$db->select('provinces.name as nama_provinsi, COUNT(t_anggota.id) AS total');
			$db->from('t_anggota');
			$db->join('provinces', 'provinces.id=t_anggota.provinsi', 'left');
			$db->where('t_anggota.is_active', 'Y');
			$db->group_by('provinces.name');
			/*excecute query*/
			$data = $db->get()->result_array();
			foreach($data as $row){
				$fields[$row['nama_provinsi']] = $row['total'];
			}
		}
